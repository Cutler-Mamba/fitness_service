...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Tenant { action } => match action {
            TenantAction::Create { wechat_user_id, nickname } => {
                let tenant = tenant_service
                    .create(&wechat_user_id, nickname.as_deref())
                    .await?;
                println!("Tenant created: id={}, wechat_user_id={}, status={}", tenant.id, tenant.wechat_user_id, tenant.status);
            }
            TenantAction::List => {
                ...
                if tenants.is_empty() {
                    ...
                    println!("{:<36} {:<32} {:<20} {:<10}", "ID", "WeChat User ID", "Nickname", "Status");
                    println!("{}", "-".repeat(100));
                    for t in &tenants {
                        println!(
                            "{:<36} {:<32} {:<20} {:<10}",
            ...
            TenantAction::Status { wechat_user_id, status } => {
                let tenant = tenant_service.set_status(&wechat_user_id, &status).await?;
                println!("Tenant status updated: wechat_user_id={}, status={}", tenant.wechat_user_id, tenant.status);
            }
            TenantAction::Show { wechat_user_id } => {
                match tenant_service.find_by_wechat_id(&wechat_user_id).await? {
                    Some(t) => {
                        ...
                        println!("Daily Quota:    {}", t.daily_quota.map_or("unlimited".to_string(), |q| q.to_string()));
